import base64
import csv

from docx import Document
from docx.shared import Inches

from core import action_hub, get_sdk_for_schedule, get_sdk_all_access, send_email
from main import app
from hub_classes import ActionDefinition, ActionList, ActionParameter, ActionRequest

action_slug = 'look_to_word'

word_icon = 'data:image/png;base64,'

definition = ActionDefinition(
                name= action_slug,
                url= f'{action_hub}/actions/{action_slug}/action',
                label= 'Word',
                icon_data_uri= word_icon,
                form_url= f'{action_hub}/actions/{action_slug}/form',
                supported_action_types= ['query'],
                description= 'This action will generate a Word document based on a Look',
                params= [],
                supported_formats= ['wysiwyg_png', 'csv'],
                supported_formattings= ['formatted', 'unformatted'],
                supported_visualization_formattings= ['apply', 'noapply'],
            )

@app.post(f'/actions/{action_slug}/form')
def form_endpoint():
    """Standard Action Hub endpoint. Returns this action's sending/scheduling form as a JSON response."""
    return [
        ActionParameter(
            name='email_address',
            label='Email Address',
            description='Email address to send Word document',
            required=True,
        ),
        ActionParameter(
            name='email_subject',
            label='Subject',
            description='Email subject line',
            required=True,
        ),
        ActionParameter(
            name='email_body',
            label='Subject',
            description='Email body text',
            required=True,
            type='textarea'
        ),
        ActionParameter(
            name='filename',
            label='Filename',
            description='Filename for the generated Word document',
            required=True,
        ),
    ]

@app.post(f'/actions/{action_slug}/action')
def action_endpoint(payload: ActionRequest):
    """Endpoint for the Look to Word action: runs the given Look and embeds in a Word doc along with its description."""
    sdk = get_sdk_for_schedule(payload.scheduled_plan.scheduled_plan_id)
    plan = sdk.scheduled_plan(payload.scheduled_plan.scheduled_plan_id)
    look = sdk.look(plan.look_id)
    image = sdk.run_look(plan.look_id, 'png')
    description = look.description

    image_file = 'temp/look.png'
    with open(image_file, 'wb') as file:
        file.write(image)
    
    document = Document()
    document.add_heading('Exported Look', 0)

    p = document.add_paragraph(description)

    document.add_picture(image_file, width=Inches(4.50))

    document.save('output/look.docx')

    response = send_email(
        from_email='contact@jonathanwalls.com',
        to_emails='jonathan.walls@looker.com',
        subject='Look to Word Test',
        body='This is the body text.',
        file_name='output/look.docx',
        file_type='docx'
    )

    return {'response': 'response'}

